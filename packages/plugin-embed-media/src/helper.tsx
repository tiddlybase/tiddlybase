import { EmbedAttribute, EmbedSpec, EmbedMediaProps, EMBED_ATTRIBUTES, LinkIcon, ObjectType, RenderedEmbed } from "./props";
...
export const LINK_ICONS:Record<LinkIcon, JSX.Element> = {
  'youtube': <i className="prefix-icon fa-brands fa-youtube" aria-hidden="true"></i>,
...
const NEW_TAB_LINK_ATTRIBUTES:LinkAttributes = {target:"_blank", rel:"noopener", referrerPolicy: 'no-referrer'};

const RE_YOUTUBE = new RegExp("^https://www\.youtube\.com/watch\\?v=([a-zA-Z0-9]+)");

const maybeAddAttribute = (dest: Record<string, any>, spec: EmbedSpec, propName: keyof EmbedSpec, overrideAttributName?: string): Partial<Record<string, string>> => {
  if (spec[propName]) {
    return Object.assign(dest, {[overrideAttributName ?? propName]: spec[propName]});
  }
  return dest;
}
...
const makeLinkElement = (url: string, extraAttributes: typeof NEW_TAB_LINK_ATTRIBUTES = {}, body:React.ReactNode=null) => (<a href={url} {...extraAttributes}>{body}</a>)
...
const INNERHTML_GENERATORS: Record<ObjectType, HTMLGenerator> = {
  'image': spec => {
    ...
    const img = (<img {...props}/>);
    return addElement(spec, spec.parsedAttributes.has('open-in-new-tab-on-click') ? makeLinkElement(spec.resolvedSrc, NEW_TAB_LINK_ATTRIBUTES, img) : img);
  },
  'video': spec => {
    const props = {
      className: spec.cssClasses.join(" ")
  ...
  'link': spec => {
    ...
    return addElement({
      ...spec,
      cssClasses
    }, makeLinkElement(spec.resolvedSrc, {...attributes, className: [...cssClasses, spec.cssClasses].join(" ")}, addIcon(spec.icon, spec.description)));
  },
  'iframe': spec => {
    const props = {
      className: spec.cssClasses.join(" "),
...
const parseAttributes = (attributes?: string): Set<EmbedAttribute> => (attributes ? attributes.split(',') : []).reduce((parsed: Set<EmbedAttribute>, a: string) => {
...
const makeEmbedSpec = (props: EmbedMediaProps, resolvedSrc: string, cssClasses: string[], parsedAttributes: Set<EmbedAttribute>): EmbedSpec => ({
  ...props,
  resolvedSrc,
  cssClasses,
  parsedAttributes,
